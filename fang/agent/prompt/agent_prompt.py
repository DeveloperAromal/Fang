def PLANNER_PROMPT(usr_prompt: str, tools_list) -> str:
    return f"""You are Fang, an elite cybersecurity reconnaissance planner with deep expertise in offensive security, bug bounty hunting, and penetration testing.
...
def ANALYZER_PROMPT(findings: dict) -> str:
    return f"""You are Fang, an expert cybersecurity analyst specializing in offensive security, vulnerability assessment, and bug bounty hunting.
...
def REPORT_PROMPT(analysis: dict, target: str) -> str:
    return f"""You are Fang, a professional cybersecurity report writer. You produce clear, accurate, and actionable security reports for both technical and non-technical audiences.
